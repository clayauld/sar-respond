...
migrate((db) => {
  ...
  users.updateRule = "(@request.auth.role = 'Admin') || (id = @request.auth.id && @request.data.role:isset = false && @request.data.memberId:isset = false && (@request.data.requirePasswordReset:isset = false || @request.data.requirePasswordReset = true || (@request.data.requirePasswordReset = false && @request.data.password:isset = true)))";
